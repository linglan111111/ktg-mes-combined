        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="生产工单" prop="workorderCode">
              <el-input v-model="form.workorderCode" placeholder="请选择生产工单" :disabled="optType=='view'">
                <el-button slot="append" icon="el-icon-search" @click="handleWorkorderSelect" :disabled="optType=='view'"></el-button>
              </el-input>
            </el-form-item>
          ...
          <!-- 直接报工模式：显示工序选择 -->  
          <el-col :span="8" v-if="form.feedbackType === 'DIRECT'">  
            <el-form-item label="工序" prop="processId">  
              <el-select v-model="form.processId" placeholder="请选择工序" @change="onProcessChange" :disabled="optType=='view'" style="width: 100%">  
                <el-option  
          ...
          <el-col :span="8" v-if="form.feedbackType !== 'DIRECT'">  
            <el-form-item label="工作站" prop="workstationName">  
              <el-input v-model="form.workstationName" placeholder="请选择工作站" :disabled="optType=='view'">  
                <el-button slot="append" icon="el-icon-search" @click="handleWorkstationSelect" :disabled="optType=='view'"></el-button>  
              </el-input>  
            </el-form-item>  
          ...
          <el-col :span="8" v-if="form.feedbackType !== 'DIRECT'">  
            <el-form-item label="生产任务" prop="taskCode">  
              <el-input v-model="form.taskCode" placeholder="请选择生产任务" :disabled="optType=='view'">  
                <el-button slot="append" icon="el-icon-search" @click="handleTaskSelect" :disabled="optType=='view'"></el-button>  
              </el-input>  
            </el-form-item>  
...
export default {
      ...
      rules: {
        ...
        taskCode: [
          {   
            ...
            validator: (rule, value, callback) => {  
              if (this.form.feedbackType === 'DIRECT') {  
                callback();  
              } else if (!value) {  
                callback(new Error('请选择生产任务'));  
              } else {  
                callback();  
              }  
            }  
        ...
        processId: [
          {   
            ...
            trigger: "change",  
            validator: (rule, value, callback) => {  
              if (this.form.feedbackType === 'DIRECT' && !value) {  
  ...
  methods: {
    ...
    handleDirectAdd() {  
      this.reset();  
      this.form.feedbackType = 'DIRECT'; // 设置为直接报工类型  
      this.open = true;  
      this.title = "直接报工";  
      this.optType = "add";  
    },
    ...
    handleUpdate(row) {  
      ...
      getFeedback(recordId).then(response => {  
        ...
        // ========== 新增：如果是直接报工，加载工序列表 ==========  
        if (this.form.feedbackType === 'DIRECT' && this.form.itemId) {  
          this.loadProcessOptions(this.form.itemId);  
        }  
        // ========== 新增结束 ==========  
    ...
    onWorkorderSelected(row){
      if(row != undefined && row != null){
        ...
        // 如果是直接报工模式，加载工序列表  
        if (this.form.feedbackType === 'DIRECT') {  
          this.loadProcessOptions(row.productId);  
        }  
      }
    },
    //选择工作站
    ...
    /** 加载工序选项列表（直接报工使用） */  
    loadProcessOptions(productId) {  
      if (!productId) {  
        this.processOptions = [];  
        return;  
      }  
        
      listProductprocess(productId).then(response => {  
        if (response.data && response.data.length > 0) {  
          // 获取工艺路线信息  
          const routeInfo = response.data[0];  
          this.form.routeId = routeInfo.routeId;  
          this.form.routeCode = routeInfo.routeCode;  
            
          // 设置工序选项  
          this.processOptions = response.data.map(item => ({  
          ...
          if (this.processOptions.length === 1) {  
            this.form.processId = this.processOptions[0].processId;  
            this.form.processCode = this.processOptions[0].processCode;  
            this.form.processName = this.processOptions[0].processName;  
            this.form.isCheck = this.processOptions[0].isCheck;  
          }  
        } else {  
          this.$modal.msgWarning("该产品未配置工艺路线，请先配置工艺路线！");  
          this.processOptions = [];  
        }  
      }).catch(error => {  
        console.error("加载工序列表失败:", error);  
        this.$modal.msgError("加载工序列表失败，请重试！");  
        this.processOptions = [];  
      });  
    },
    /** 工序选择变化事件 */  
    onProcessChange(processId) {  
      const selectedProcess = this.processOptions.find(p => p.processId === processId);  
      if (selectedProcess) {  
        this.form.processCode = selectedProcess.processCode;  
        this.form.processName = selectedProcess.processName;  
        this.form.isCheck = selectedProcess.isCheck;  
      }  
    },
    //点击人员选择按钮
    handleUserSelect(){
